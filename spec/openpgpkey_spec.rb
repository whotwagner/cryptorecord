...
describe Cryptorecord::Openpgpkey do
  
  it "empty instances can be created" do
    sshfp = Cryptorecord::Openpgpkey.new
  end
 ...
 it "set uid to a hash-object should raise Cryptorecord::ArgumentError" do
   expect{sshfp = Cryptorecord::Openpgpkey.new(:uid => Hash.new)}.to raise_error(Cryptorecord::ArgumentError)
 end
